...
- **意识体记忆与关系温度：** 可按每位住客开启或关闭的**持续记忆**；**记忆向量与检索**（在**本机**或**你所信任的云端**嵌入模型之间择一，密钥不离开你的设备与约定）；**对话后的轻量小结与分析**、把对白的要点**接回**可回看的记忆脉络；**首驻与动态**关系分、**秘境中的「重建关系」**；**关系分档**自「**陌生人**」起，最高至如「**亲密相知**」等温柔档位（**最高档具体称谓**会随你俩性别与组合，换成更贴脸的叫法）。  
- **材料与故事：** `.md` / `.txt` 导入、自动语言识别、**记忆秘藏**、灵魂创建后**再写入新材料**的**融合式更新**；常见 **IM 聊天导出**（**微信、QQ、飞书、钉钉** 等，各平台格式以应用实际支持为准）的**解析、预览与接入**，可接进材料与秘藏。  
- **联网与寻光：** 对话中可选 **LLM 原生 / Brave / Firecrawl** 等检索与工具环（Key **本机密文**、经同机**代理**代发、**URL 白名单**可照看你关心的站点）。**梦幻同伴**侧「**打破次元壁**」的检索与素材，**可先落进召唤草稿**再与你共同定稿。  
- **导出：** ZIP / 单文件 Markdown、面向 **OpenClaw** 的**分步引导导出**（可携带对话期记忆，依导出指引勾选）。  
