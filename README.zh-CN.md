...
**梦幻同伴**为那些**只活在作品里**的 TA 而开——来自小说、番剧、游戏、银幕的某个侧影。你不必先把设定背得完美：入门处会有**召唤之门**，你可以只写下名字与来处，在模型能力允许时，尝试**籍籍无名式的一键召唤**；也可以轻轻推开「**我自己来**」，沿问卷**一步一步**把光聚回来。  

我们会陪你走过**认识 TA → 个性与说话方式 → 情绪与底线 → 你们如何相处**；在这之后，是只属于梦幻同伴的**「打破次元壁」**——在已配置大模型与联网能力的前提下，依你在**设置**里信得过的**搜索与站点白名单**，为 TA 从公开网络借几笔**世界与台词**的参照（**你可以随时跳过**）。接着是**更多素材**与文本整理，所有检索结果**只落在你本机**，为后面的五文档**灵魂提取**垫一层更厚的尘土与星光。  
